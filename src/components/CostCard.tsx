...
import type {CostBreakdown, ParameterConfig}
...
export function CostCard({
    ...
    const handlePrint = () => {
      ...
      <html>
        ...
        <body>
          ...
          <div class="breakdown">
            ${
            parameterConfig.enabled.pricePerKg ? `<div class="breakdown-item"><span>${
                UI_TEXT.COST_DETAILS.MATERIAL_COST
            }</span><span>${
                formatCurrency(costs.materialCost)
            }</span></div>` : ''
        }
            ${
            parameterConfig.enabled.pricePerHour ? `<div class="breakdown-item"><span>${
                UI_TEXT.COST_DETAILS.TIME_COST
            }</span><span>${
                formatCurrency(costs.printTimeCost)
            }</span></div>` : ''
        }
            ${
            (parameterConfig.enabled.electricityConsumption && parameterConfig.enabled.electricityPrice) ? `<div class="breakdown-item"><span>${
                UI_TEXT.COST_DETAILS.ELECTRICITY_COST
            }</span><span>${
                formatCurrency(costs.electricityCost)
            }</span></div>` : ''
        }
            ${
            parameterConfig.enabled.flatWorkFee ? `<div class="breakdown-item"><span>${
                UI_TEXT.COST_DETAILS.WORK_FEE
            }</span><span>${
                formatCurrency(costs.flatWorkFee)
            }</span></div>` : ''
        }
            <div class="breakdown-item breakdown-subtotal"><span>${
            UI_TEXT.COST_DETAILS.SUBTOTAL
        }</span><span>${
            formatCurrency(costs.subtotal)
        }</span></div>
            ${
            parameterConfig.enabled.markup ? `<div class="breakdown-item"><span>${
                parameterConfig.useDiscount ? UI_TEXT.COST_DETAILS.DISCOUNT_LABEL(parameterConfig.value.markup) : UI_TEXT.COST_DETAILS.MARKUP_LABEL(parameterConfig.value.markup)
            }</span><span>${
                formatCurrency(costs.markupAmount)
            }</span></div>` : ''
        }
            <div class="breakdown-item breakdown-total"><span>${
            UI_TEXT.COST_DETAILS.TOTAL
        }</span><span>${
    ...
    return (
        <div className="rounded-lg p-6 print-area"
            style={
                {
            ...
            <div className="rounded-lg p-4 mb-4"
                ...
                <div className="text-center">
                    <p className="text-sm"
                        ...
                        {
                        UI_TEXT.COST_DETAILS.TOTAL_COST
                    }</p>
            ...
            <button onClick={
                ...
                <span className="flex items-center justify-between">
                    <span style={
                        ...
                        {
                        UI_TEXT.COST_DETAILS.SHOW_DETAILS
                    }</span>
                    <span className={
