...
import { ParameterConfig, Parameters } from '../types';

interface ParametersProps {
  config: ParameterConfig;
  onToggleParameter: (key: keyof Parameters) => void;
  onEditClick: () => void;
}
    ...
    <div className="bg-gray-100 border border-gray-300 rounded-lg p-6">
      ...
      <div className="space-y-3">
        {Object.entries(config.value).map(([key, value]) => {
          const enabled = config.enabled[key as keyof Parameters];
          const parameterKey = key as keyof Parameters;
            ...
            >
              <div className="flex items-center gap-3">
                <input
                  type="checkbox"
                  checked={enabled}
                  onChange={() => onToggleParameter(parameterKey)}
